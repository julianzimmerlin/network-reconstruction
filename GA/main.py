...
scores, losses, dynamics_learners, optimizers = evaluator.evaluate_population(population, NUM_DYN_EPOCHS_INIT, dynamics_learners, optimizers, True)
...
for j in range(NUM_GEN):
    ...
    for i in range(0, NEWPOP_SIZE, 2):
        ...
        # mutate
        newpop[i] = su.mutate_matrix_guided(newpop[i], dynamics_learners[idx[i]], evaluator)
    ...
    newscores, newlosses, newdynamics_learners, newoptimizers = evaluator.evaluate_population(newpop,
                                   NUM_DYN_EPOCHS_INIT if j % RESET_DYN_LEARNER_EVERY_NTH_GEN == 0 else NUM_DYN_EPOCHS,
                                  [dynamics_learners[id] for id in idx], [optimizers[id] for id in idx], True)
    scores, losses, dynamics_learners, optimizers = evaluator.evaluate_population(population,
                                   NUM_DYN_EPOCHS_INIT if j % RESET_DYN_LEARNER_EVERY_NTH_GEN == 0 else NUM_DYN_EPOCHS,
                                   dynamics_learners, optimizers, True)
    # update population
